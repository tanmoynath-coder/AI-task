...
mod tests {
    ...
    fn test_path_traversal_dots() {
        ...
        // Canonicalized check should fail:
        // This test documents the vulnerability - canonicalize() needed in production
    }

    #[test]
    fn test_path_traversal_symlink() {
        // Symlink attack: data/tools/malicious -> /etc/passwd
        // Integration test would require temp dir setup
        // Documented as test case for future implementation
    }
    ...
    fn test_path_with_null_bytes() {
        // Rust's PathBuf handles null bytes safely, but test edge case
        let path = PathBuf::from("data/tools/test\0.yml");
        assert!(path.exists() == false); // Should not exist
    }

    // ============================================================
    // Security Tests: Error Message Handling
    // ============================================================

    #[test]
    fn test_error_message_no_panic_on_empty() {
        let empty_body = "";
        let msg = format!("Request failed: {}", empty_body);
        assert!(msg.contains("Request failed"));
        // Ensure no unwrap/expect on empty responses
    }
    ...
    // ============================================================
    // Functional Tests: CheckResult Logic
    // ============================================================
    ...
    fn test_tool_report_any_fail() {
        let all_pass = ToolReport {
            ...
            stars: CheckResult::Pass("30 stars".to_string()),
            contributors: CheckResult::Pass("5 contributors".to_string()),
            age: CheckResult::Pass("100 days".to_string()),
            note: None,
        };
        ...
        let one_fail = ToolReport {
            name: "test".to_string(),
            source: Some("https://github.com/owner/repo".to_string()),
            stars: CheckResult::Fail("5 stars".to_string()),
            contributors: CheckResult::Pass("5 contributors".to_string()),
            age: CheckResult::Pass("100 days".to_string()),
            note: None,
        };
    ...
    // ============================================================
    // Functional Tests: Contributing Criteria Thresholds
    // ============================================================
    ...
    // ============================================================
    // Functional Tests: ToolEntry Parsing
    // ============================================================
...
"#;
    ...
    #[test]
    fn test_tool_entry_without_source() {
        let yaml = r#"
name: proprietary-tool
"#;
        let entry: ToolEntry = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(entry.name, "proprietary-tool");
    ...
    fn test_url_fragment() {
        ...
        assert_eq!(result, None); // Should reject URLs with fragments
    }

    #[test]
    fn test_url_query_params() {
        let result = parse_github_repo("https://github.com/owner/repo?tab=security");
        assert_eq!(result, None); // Should reject URLs with query params
    }

    #[test]
    fn test_empty_owner() {
        let result = parse_github_repo("https://github.com//repo");
    ...
    fn test_empty_repo() {
        let result = parse_github_repo("https://github.com/owner/");
        assert_eq!(result, Some(("owner".into(), "".into())));
        // Note: This currently passes but should be rejected
        // TODO: Add validation for empty repo name
    }

    #[test]
    fn test_special_characters_in_repo() {
        let result = parse_github_repo("https://github.com/owner/repo-name");
        assert_eq!(result, Some(("owner".into(), "repo-name".into())));
    }

    #[test]
    fn test_uppercase_owner_repo() {
        let result = parse_github_repo("https://github.com/OWNER/REPO");
        assert_eq!(result, Some(("OWNER".into(), "REPO".into())));
        // GitHub is case-insensitive, but we preserve case
    }
}
